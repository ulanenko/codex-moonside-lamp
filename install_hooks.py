...
def main() -> int:
    ...
    if args.write_hooks:
        wrote = write_hooks(CODEX_DIR / "hooks.json", args.hook_command or default_hook_command(), force=args.force)
        print(("Wrote " if wrote else "Already exists: ") + str(CODEX_DIR / "hooks.json"))

    if args.print_hooks or not (args.write_config or args.write_hooks):
        hooks = load_hooks_template(args.hook_command or default_hook_command())
